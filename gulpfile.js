...
//TODO: test and tweak
/*gulp.task('codesign', () => {
    exec('sh dist/mac/codesign.app || echo "Codesign failed, likely caused by not being run on mac, continuing"', (error, stdout, stderr) => {
        console.log(stdout);
    });
});
*/
  ...
  return Promise.all(
    nw.options.platforms.map((platform) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        const child = spawn('bash', ['dist/mac/pkg-maker.app']);
