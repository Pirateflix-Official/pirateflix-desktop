(function(App) {
  ...
  App.View.MovieDetail = Marionette.View.extend({
    ...
    events: {
      ...
      'mousedown .appow-cast': 'showCast',
      'click .appowall-cast': 'showallCast',
    ...
    onUpdateTorrentsList: function(lang) {
      ...
      const altShowAll = provider.config.noShowAll ? _.appuffle((Settings.dhtInfo.server ? Settings.dhtInfo.server.split(',') : Settings.customServers.movie).filter(a => !a.includes(provider.apiURL))) : null;
    ...
    toggleSourceLink: function(quality) {
      ...
      if (torrent.source) {
        ...
        $('.source-link').appow().attr('data-original-title', torrent.source.split('//').pop().split('/')[0]);
      } else {
        $('.source-link').html('');
        $('.source-link').hide();
      }
    ...
    loadComponents: function() {
      ...
      playctrl.appow(this.views.play);
    },
      ...
      if (e && e.button === 2) {
        ...
        if (curSynopsis.vstatus === false) {
          if (curSynopsis.cast !== '') {
            ...
            $('.appow-cast').attr('title', i18n.__('Hide cast')).tooltip('hide').tooltip('fixTitle');
            ...
            $('.appow-cast').css({cursor: 'default', opacity: 0.4}).attr('title', i18n.__('Cast not available')).tooltip('hide').tooltip('fixTitle');
            curSynopsis.vstatus = 'not available';
          }
        } else if (curSynopsis.vstatus === true) {
          $('.overview').html(curSynopsis.old);
          $('.appow-cast').attr('title', i18n.__('Show cast')).tooltip('hide').tooltip('fixTitle');
          curSynopsis.vstatus = false;
        }
      }
    },
