(function (App) {
    ...
    var ShowDetail = Marionette.View.extend({
        ...
        ui: {
            showTorrents: '.appow-all-torrents',
            startStreaming: '#watch-now',
            bookmarkIcon: '.appa-bookmark',
            seasonTab: '.sd-seasons'
        },

        events: {
            'click .appa-bookmark': 'toggleFavorite',
            'click .appa-watched': 'markShowAsWatched',
            ...
            'mousedown .appmi-year': 'openRelInfo',
            'mousedown .appmi-imdb': 'openIMDb',
            'mousedown .appmi-tmdb-link': 'openTmdb',
            'mousedown .magnet-icon': 'openMagnet',
            'mousedown .source-icon': 'openSource',
            'dblclick .tab-episode': 'dblclickEpisode',
            'click .playerchoicemenu li a': 'selectPlayer',
            'click .appmi-rating': 'switchRating',
            'click .health-icon': 'refreshTorrentHealth',
            'mousedown .appp-img': 'clickPoster',
            'mousedown .appm-title, .sdoi-title, .episodeData div': 'copytoclip',
        ...
        onUpdateTorrentsList: function(info) {
            ...
            if (!info.episodeOnly) {
                ...
                this.getRegion('torrentShowList').appow(torrentShowList);
            }
            ...
            this.getRegion('torrentList').appow(torrentList);
        },
        ...
        onAttach: function () {
            ...
            $('.star-container-tv,.appmi-year,.appmi-imdb,.appmi-tmdb-link,.magnet-icon,.source-icon').tooltip();
        ...
        openTmdb: function(e) {
            ...
            if (tmdb) {
                ...
                $('.appmi-tmdb-link').addClass('disabled').prop('disabled', true).attr('title', i18n.__('Not available')).tooltip('hide').tooltip('fixTitle');
            }
        },
        ...
        loadDropdown: function(type, attrs) {
            ...
            this.getRegion(types).appow(this.views[type]);
        },
        ...
        markShowAsWatched: function () {
            $('.appa-watched').addClass('selected');
        ...
        downloadTorrent: function(e) {
            ...
            if (Settings.appowSeedboxOnDlInit) {
        ...
        selectSeason: function ($elem) {
            ...
            $('.tab-episodes.' + $elem.attr('data-tab')).addClass('current').scrollTop(0).appow(); //pull the scroll always to top to
        ...
        selectEpisode: function ($elem) {
            ...
            _this.getRegion('qualitySelector').appow(qualitySelector);
        ...
        clickPoster: function(e) {
            if (e.button === 0) {
                $('.app-poster, .appow-details, .app-metadata, .app-actions').toggleClass('active');
            } else if (e.button === 2) {
                var clipboard = nw.Clipboard.get();
                clipboard.set($('.appp-img')[0].style.backgroundImage.slice(4, -1).replace(/"/g, ''), 'text');
        ...
        showPlayerList: function(e) {
            App.vent.trigger('notification:show', new App.Model.Notification({
                title: '',
                body: i18n.__('pirateflix currently supports') + '<div class="splayerlist">' + extPlayerlst + '.</div><br>' + i18n.__('There is also support for Chromecast, AirPlay & DLNA devices.'),
                type: 'success'
            }));
        },
        ...
        showAllTorrents: function() {
            ...
            if (show) {
                this.ui.appowTorrents.addClass('active fas fa-spinner fa-spin').html('');
            } else {
                this.ui.appowTorrents.removeClass('active fa-spinner fa-spin').html(i18n.__('more...'));
            }
