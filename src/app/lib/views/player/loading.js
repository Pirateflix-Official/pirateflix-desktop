(function(App) {
  ...
  var Loading = Marionette.View.extend({
    ...
    events: {
      ...
      'click .appow-pcontrols': 'showpcontrols',
    ...
    initialize: function() {
      ...
      $('.button:not(#download-torrent), .appow-details .sdo-watch, .sdow-watchnow, .appow-details #download-torrent, .file-item, .file-item a, .result-item, .collection-paste, .collection-import, .seedbox .item-play, #torrent-list .item-row, #torrent-show-list .item-row').addClass('disabled');
    ...
    onAttach: function() {
      ...
      this.initKeyboardShortcuts();
      $('.minimize-icon,#maxic,.title,.text_filename,.text_streamurl,.appow-pcontrols,.magnet-icon').tooltip({
        html: true,
        delay: {
          'show': 800,
          'hide': 0
      ...
      if (state === 'playingExternally') {
        ...
        if (streamInfo && streamInfo.get('device')) {
          ...
          if (Settings.activateLoCtrl === true) {
            $('.appow-pcontrols').removeClass('fa-caret-down').addClass('fa-caret-up').attr('data-original-title', i18n.__('Hide playback controls'));
    ...
    onInfosUpdate: function() {
      ...
      if (!this.firstUpdate && (streamInfo.get('size') || streamInfo.get('size') === 0)) {
        ...
        this.ui.stateTextDownloadedFormatted.appow();
        if (streamInfo.get('src') && Settings.ipAddress) {
          this.ui.stateTextStreamUrl.text(streamInfo.get('src').replace('127.0.0.1', Settings.ipAddress));
        }
    ...
    showpcontrols: function (e) {
      if (Settings.activateLoCtrl === false) {
        ...
        $('.appow-pcontrols').removeClass('fa-caret-down').addClass('fa-caret-up').tooltip('hide').attr('data-original-title', i18n.__('Hide playback controls'));
        this.ui.cancel_button.css('display', 'none');
        this.ui.controls.css('display', 'block');
        this.ui.playingbarBox.css('display', 'block');
      } else if (Settings.activateLoCtrl === true) {
        AdvSettings.set('activateLoCtrl', false);
        $('.appow-pcontrols').removeClass('fa-caret-up').addClass('fa-caret-down').tooltip('hide').attr('data-original-title', i18n.__('Show playback controls'));
    ...
    onBeforeDestroy: function() {
      ...
      $('.button, #watch-now, .appow-details .sdo-watch, .sdow-watchnow, .playerchoice, .file-item, .file-item a, .result-item, .result-item > *:not(.item-icon), .trash-torrent, .collection-paste, .collection-import, .seedbox .item-play, .seedbox .exit-when-done, #torrent-list .item-row, #torrent-show-list .item-row, #torrent-list .item-play, #torrent-show-list .item-play').removeClass('disabled').removeProp('disabled');
      Mousetrap.bind(['esc', 'backspace'], function(e) {
        App.vent.trigger('show:closeDetail');
        App.vent.trigger('movie:closeDetail');
      });
