    ...
    return Promise.all(
      items.map(function(item) {
        if (item.next_episode) {
          ...
          show.imdb_id = item.appow.ids.imdb;
          show.tvdb_id = item.appow.ids.tvdb;
          show.rating = item.appow.rating;
          show.title = item.appow.title;
          show.trailer = item.appow.trailer;
  ...
  var load = function() {
    ...
    return App.Trakt.client.ondeck
      ...
      .then(function(tv) {
        ...
        watchlist = watchlist.concat(tv.appows);
